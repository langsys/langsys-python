...
MUTATIONS = [
    ...
    m("SNAP-3", "the checksum drifts from PHP's bytes", "src/langsys/snapshot.py",
      '            return [] if not node else {k: php_shape(v) for k, v in node.items()}',
      '            return {k: php_shape(v) for k, v in node.items()}',
      "test_SNAP_the_checksum_is_byte_identical_to_the_php_sdks"),
    # -- CACHE / OBS --------------------------------------------------------------------------
