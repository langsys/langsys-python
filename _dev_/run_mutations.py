...
MUTATIONS = [
    ...
    # -- MSG ----------------------------------------------------------------------------------
    m("MSG-1", "an object with neither template nor message counts as an entry", "src/langsys/messages.py",
      '    if not isinstance(template, str) and not isinstance(message, str):\n        return None',
      '    if False:\n        return None',
      "test_MSG1_entry_resolution_matches_the_vectors[neither-template-nor-message-is-not]"),
    m("MSG-1", "an entry with only a message is dropped", "src/langsys/messages.py",
      '    if not isinstance(template, str) and not isinstance(message, str):\n        return None',
      '    if not isinstance(template, str):\n        return None',
      "test_MSG1_entry_resolution_matches_the_vectors[message-without-template-is-an-entry]",
      "test_MSG1_an_entry_with_no_template_is_not_looked_up_and_shows_its_message"),
    m("MSG-1", "the framework's body is searched instead of the configured key", "src/langsys/messages.py",
      '    found = resolver(body) if resolver is not None else _dig(body, key or "")',
      '    found = resolver(body) if resolver is not None else body',
      "test_MSG1_entry_resolution_matches_the_vectors[framework-body-not-searched]",
      "test_MSG1_the_body_is_never_searched_by_shape"),
    m("MSG-1", "configured piece names are ignored", "src/langsys/messages.py",
      '        return value.get(names.get(piece, piece))',
      '        return value.get(piece)',
      "test_MSG1_entry_resolution_matches_the_vectors[rails-body-renamed-pieces]"),
    m("MSG-1", "a field -> entries map at the key is not read", "src/langsys/messages.py",
      '    if values and all(isinstance(v, list) for v in values):',
      '    if False:',
      "test_MSG1_entry_resolution_matches_the_vectors[drf-body-nested-key-field-map]"),
    m("MSG-1", "attaching overwrites a member of the framework's body", "src/langsys/messages.py",
      '    if key in body:\n        raise ValueError',
      '    if False:\n        raise ValueError',
      "test_MSG1_the_attach_key_is_configurable_and_never_overwrites"),
    m("MSG-1", "an entry without message has no fallback", CLIENT,
      '            return fill_template(template, entry.get("params")) if isinstance(template, str) else ""',
      '            return ""',
      "test_MSG1_an_entry_needs_only_a_template_and_its_params"),
    m("MSG-2", "the framework's code is replaced by an SDK code", "src/langsys/messages.py",
      '        entry["field"] = field\n    if code is not None:\n        entry["code"] = code',
      '        entry["field"] = field\n    if code is not None:\n        entry["code"] = "invalid"',
      "test_MSG2_the_frameworks_code_and_field_path_pass_through_unchanged",
      "test_MSG2_the_code_is_unchanged_across_locales_and_never_chooses_text"),
    m("MSG-2", "the framework's field path is flattened", "src/langsys/messages.py",
      '    if field is not None:\n        entry["field"] = field',
      '    if field is not None:\n        entry["field"] = ".".join(map(str, field)) if isinstance(field, list) else field',
      "test_MSG2_the_frameworks_code_and_field_path_pass_through_unchanged"),
    m("MSG-2", "a failure with no identifier is given one", "src/langsys/messages.py",
      '        entry["field"] = field\n    if code is not None:\n        entry["code"] = code',
      '        entry["field"] = field\n    if True:\n        entry["code"] = code',
      "test_MSG2_a_failure_with_no_identifier_carries_no_code"),
    ...
    m("MSG-5", "an entry with no template shows nothing", CLIENT,
      '            if isinstance(message, str):\n                return message',
      '            if False:\n                return message',
      "test_MSG5_rendering_matches_the_vectors[no-template-shows-message]"),
    ...
    m("MSG-7", "problems fail the run without --strict", "src/langsys/messages.py",
      '    return 1 if strict and listing.problems else 0',
      '    return 1 if listing.problems else 0',
      "test_MSG7_an_unlistable_message_is_reported_and_the_run_still_exits_zero"),
    m("MSG-7", "--strict does not fail the run", "src/langsys/messages.py",
      '    return 1 if strict and listing.problems else 0',
      '    return 0',
      "test_MSG7_strict_fails_the_run_on_any_unlistable_message"),
    ...
    m("MSG-11", "a framework label placeholder is accepted", "src/langsys/messages.py",
      '            if placeholder in template:',
      '            if False:',
      "test_MSG11_a_framework_label_placeholder_is_refused_when_added[field-label]"),
    m("MSG-11", "the template-language placeholder is not in the defaults", "src/langsys/messages.py",
      '    "%(field)s", "{{ field }}",',
      '    "%(field)s",',
      "test_MSG11_a_framework_label_placeholder_is_refused_when_added[template-field]"),
    m("MSG-11", "a binding's own placeholders are ignored", "src/langsys/messages.py",
      '        for placeholder in self.label_placeholders:',
      '        for placeholder in DEFAULT_LABEL_PLACEHOLDERS:',
      "test_MSG11_a_binding_names_its_own_frameworks_placeholders"),
    ...
    m("MSG-11", "no warning for a catalogued marker value", CLIENT,
      ...
      "test_MSG11_a_marker_filled_with_a_catalogued_phrase_warns_once"),
    # -- SRV-6 / GATE-10 ----------------------------------------------------------------------
    m("SRV-6", "the framework's locale is ignored", "src/langsys/request_locale.py",
      '    if framework is not None and str(framework).strip():',
      '    if False:',
      "test_SRV6_the_frameworks_locale_is_served_whatever_the_request_says_and_adds_no_vary[bcp47]"),
    m("SRV-6", "a framework locale is not put in the project's form", "src/langsys/request_locale.py",
      '    wanted = normalize_locale(str(candidate))',
      '    wanted = str(candidate).strip().lower()',
      "test_SRV6_the_frameworks_locale_is_served_whatever_the_request_says_and_adds_no_vary[posix]"),
    m("SRV-6", "a framework locale is served unvalidated", "src/langsys/request_locale.py",
      '    if wanted in served:\n        return wanted',
      '    if True:\n        return wanted',
      "test_SRV6_a_framework_locale_the_project_does_not_serve_serves_the_base_not_the_request"),
    m("SRV-6", "an unserved framework locale falls to the SDK's negotiation", "src/langsys/request_locale.py",
      '        return LocaleChoice(chosen or project_base, "framework", ())',
      '        if chosen:\n            return LocaleChoice(chosen, "framework", ())',
      "test_SRV6_a_framework_locale_the_project_does_not_serve_serves_the_base_not_the_request"),
    m("SRV-6", "a bare framework language is not mapped through default_locales", "src/langsys/request_locale.py",
      '        default = default_locales.get(wanted)',
      '        default = None',
      "test_SRV6_a_bare_framework_language_maps_through_the_projects_default_locales",
      "test_SRV6_the_client_maps_a_bare_framework_language_through_the_project"),
    m("SRV-6", "the framework's choice adds a Vary", "src/langsys/request_locale.py",
      '        return LocaleChoice(chosen or project_base, "framework", ())',
      '        return LocaleChoice(chosen or project_base, "framework", ("Accept-Language",))',
      "test_SRV6_the_frameworks_locale_is_served_whatever_the_request_says_and_adds_no_vary[project]"),
    m("SRV-6", "the client does not pass the project's default_locales", CLIENT,
    ...
    m("SRV-6", "a header-chosen response carries no Vary", "src/langsys/request_locale.py",
      ...
      '        return LocaleChoice(normalize_locale(header_locale), "accept-language", ())',
