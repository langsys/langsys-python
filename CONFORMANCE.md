...
| CID-3 | implemented | n/a (pure) | `test_custom_id` — JS code-unit form + both PHP pipe-join spellings, deduped, never emitted. `test_registration` resolves a **live-observed** pre-correction id (`36efd6e4…`) |
...
`legacy_custom_ids` includes the JS core's code-unit MD5. It was ported from the
implementation in `langsys-js-typescript` `src/utils.ts` — not from a description of it,
which the spec warns gets Latin-1 right and CJK, Cyrillic, Greek, Hebrew and Arabic
wrong — and **validated against that implementation by executing it**: 19/19 vectors,
covering all five of those scripts plus non-BMP, the `U+2028` row, and both documented
collision pairs (`xxxA`/`xxxŁ`, `UI|Buy now` vs `UI` + `Buy now`). A test pins the
distinguishing property directly: the code-unit hash agrees with the byte hash on ASCII
and must **diverge** above it, so a port that silently became a byte hash fails.
