...
| **Profiles** | all, server |
| **specVersion** | 8.2.18 (committed, unpublished) |
| **Spec revision read** | langsys2 a95af2c2596d5a882473d9ef09d232eb5c1d7a12, docs/sdk-spec.mdx blob 5d7e6890b733a50fb6f5f5c30e0056c6ef7bcf45 |
| **Re-derive** | `git -C ~/Documents/dev/langsys2 rev-parse a95af2c2596d5a882473d9ef09d232eb5c1d7a12:docs/sdk-spec.mdx` — by commit, never by branch; re-derived at this write |
...
| **Suite** | 1014 tests in 30 files — 999 unit (contract rows included) + 15 live |
...
| SRV-6 | implemented | n/a (pure) | `test_request_locale::test_SRV6_*`. A locale the framework or app already resolved (`framework=`: Django's `request.LANGUAGE_CODE`, an app middleware's value) is served: `es-ES` and `es_ES` map to `es-es`, a bare `es` maps through authorization's `default_locales` (resolver and client against the contract double), an unsupported one is served as the base, and the SDK adds no `Vary` whatever the URL, cookie or header say. Only when nothing resolved it does the SDK resolve it itself: the spec's four requests (URL wins with no `Vary`; cookie wins with `Vary: Cookie`; header with `Vary: Accept-Language`; an unsupported cookie falls through and is not re-set), every candidate validated against the project's own locales. When authorization is unavailable, a loaded snapshot's base and locales are the served set (`test_snapshot::test_SRV6_offline_*`), and with no snapshot only the configured base. Mutations: the framework's locale ignored; `es_ES` not mapped; a framework locale unvalidated; an unserved one falls to the SDK's negotiation; no `default_locales` mapping, in the resolver and in the client; a `Vary` added to the framework's choice; header before cookie; URL or cookie unvalidated; no `Vary`; an unreadable project serves what was asked; a snapshot's locales not served offline → red |
| MSG-1 | implemented | n/a (pure) | `server-message-vectors.json` (blob `7333e391`): every `resolve` row through its configured key and piece names, with the body unchanged (`test_messages::test_MSG1_entry_resolution_matches_the_vectors[*]`). `server_message(template, params, field=, code=)` needs only the template and its params, with `message` their fill; `attach_server_messages` adds the entries beside Django's and Pydantic's native error bodies, which are otherwise unchanged, under `langsys_errors` or a configured key, and refuses to overwrite a member; entries resolve only through a key or a resolver, never by the body's shape; an entry with no template is not looked up and shows its `message`. Mutations: an object with neither piece is an entry; a message-only entry dropped; the body searched instead of the key; piece names ignored; a field → entries map unread; attaching overwrites; no fallback without `message` → red |
| MSG-2 | implemented | n/a (pure) | The framework's `code` and its `field` path (Pydantic's `loc` list included) pass through unchanged, a failure with no identifier carries no code, the code is unchanged across locales and never chooses the text, and the SDK exports no vocabulary or wording of its own (`test_messages::test_MSG2_*`); the `render` vector `code-does-not-choose-text`. Mutations: the code replaced by an SDK code; a field path flattened; a code given where there is none → red |
| MSG-3 | implemented | n/a (pure) | Every `markers` vector; every canonical entry (real Laravel, Pydantic, Django, DRF and Rails sentences with the label written in) builds to itself; one template per field is two phrases; a template with no marker is its message and carries no params. Mutations: a capitalised name as a marker; params on a marker-less template → red |
...
| MSG-7 | implemented | contract | `test_messages::test_MSG7_the_listing_registers_every_template_and_a_second_run_nothing_new` against the double; `…_an_unlistable_message_is_reported_and_the_run_still_exits_zero` (an actionable line naming the source, the field and the fix); `…_strict_fails_the_run_on_any_unlistable_message`; the `python -m langsys.messages` entry point, `--strict` included. Declarations come from a provider the binding supplies. Mutations: re-register what the catalog holds; problems fail without `--strict`; `--strict` does not fail → red |
...
| MSG-9 | n/a (architecture: a core has no validator; entries built from a failed validator are the Django and FastAPI bindings') | - | The core supplies `server_message` for the framework's unfilled sentence and its values, and `TemplateProblem` for a text-only failure the listing reports |
| MSG-10 | n/a (architecture: a core has no label facility; `verbose_name` and `Field(title=…)` are the bindings') | - | The listing command reports what a provider names as unlabelled, without failing |
| MSG-11 | implemented | n/a (pure) | Refused at add: the framework's own label placeholders — Django's `%(field_label)s`, `%(field_labels)s`, `%(date_field_label)s`, `%(model_name)s`, `%(field)s` and the template language's `{{ field }}` by default, and whatever a binding names (`label_placeholders=`) (`test_messages::test_MSG11_a_framework_label_placeholder_is_refused_when_added[*]`, `…_a_binding_names_its_own_frameworks_placeholders`); the framework's sentences with labels written in, `{name}` markers and percent prose accepted; a catalogued marker value warns once, an unseen one stays silent. Mutations: placeholders accepted; `{{ field }}` dropped from the defaults; a binding's placeholders ignored; warning not deduplicated; no warning → red |
| MSG-12 | n/a (architecture: a core has no redirect or session; a binding whose framework redirects carries entries across it) | - | |
...
| `tests/fixtures/canonicalization-reference.json` | blob `34034931872b93e761faea49fb040f3fd8a6b9f5` | `langsys-js-typescript` `a639ae8`, 32 rows, authored against 8.2.15 (the TOK and MARK text is byte-identical at 8.2.18) |
...
| `tests/fixtures/server-message-vectors.json` | blob `7333e3919dac43af81c6c20bfdba974efd79725b` | `langsys-js-typescript` `239166a`, authored against 8.2.18: 12 canonical entries, 13 markers, 10 fill, 10 resolve, 12 render |
...
(SRV-6), `server_message()` / `attach_server_messages()` / the listing command (MSG-9, MSG-10, MSG-12), and
